...
STRUCTURED_RUNTIME_STATE_ADDENDUM = (
    ...
    "- `transitions` is a chronological list of actual action transitions, excluding the initial seeded frame. Each transition exposes `.action`, `.step` (the post-action step number), `.before_frame`, `.after_frame`, `.frame` (alias of `.after_frame`), and `.result`.\n"
    "- `transitions.steps_by_action()` maps each action name to the chronological steps where it was used, e.g. `{'SPACE': [5, 10, 37], 'LEFT': [1, 5, 7]}`. `transitions.for_action('SPACE')` returns just that action's transitions, and `transitions.at_step(37)` returns the single transition that produced step 37 (or `None`).\n"
    "- When an action's effect seems confusing or inconsistent, pull all of its past transitions with `for_action` and compare their `.diff`s across different board states -- summarizing one action's effect over many uses is the fastest way to generalize its rule into the action model.\n"
