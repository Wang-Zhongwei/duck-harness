...
STRUCTURED_RUNTIME_STATE_ADDENDUM = (
    ...
    "- Use `last_transition.diff` for the latest cell-level change, or `frame_diff(before_frame, after_frame)` (short alias: `diff`) for any two equal-sized frames. The result contains `cells_changed` and `groups` ordered by decreasing size. Each group has `from`, `to`, `count`, an inclusive `bbox` as `[[r0, c0], [r1, c1]]`, and all coordinate `cells`. Printing replaces `cells` with an explicit inspection hint for groups larger than 12, but the complete list remains accessible as `group['cells']`. This preserves isolated 1-pixel changes without dumping large objects.\n"
...
PYTHON_ADDENDUM = (
    ...
    "- For frame diffs, start from `last_transition.diff`; its grouped summary preserves small-cell detail and keeps full coordinates available for deeper inspection. Then infer appearing/disappearing components or movement only when useful.\n"
