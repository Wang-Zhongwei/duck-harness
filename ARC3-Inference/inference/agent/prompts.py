...
STRUCTURED_RUNTIME_STATE_ADDENDUM = (
    ...
    "- Each node is one 4-connected same-color object. Fields, with exact types: `id` (int, top-most-left-most order), `color` (single ARC color char), `hash` (str), `area` (int, number of cells excluding children), `bbox` (`[r0, c0, r1, c1]`, inclusive), `centroid` (`[r, c]`), `h`/`w` (ints), `boundary` (clockwise perimeter corner cells), `children` (list of ids of objects fully enclosed by this one).\n"
    "- `segmentation['adjacency_list']` is a list of `[i, j]` node-id pairs whose objects share an edge.\n"
    "- Find objects with e.g. `segmentation.find(color='B', area=24).one()` -- `.one()` errors unless exactly one match, so use it whenever you expect a unique object. `segmentation.find(color='R')` returns a plain list in id order (`.first()` for the top-left-most).\n"
    "- `find()` accepts exactly these keywords and no others: `color=`, `not_color=`, `area=`, `min_area=`, `max_area=`, `id=`, `h=`, `w=`, `min_h=`/`max_h=`, `min_w=`/`max_w=`, `hash=`, `in_bbox=(r0, c0, r1, c1)`. Each matches the node field of the same name; color keywords accept a char or a set.\n"
